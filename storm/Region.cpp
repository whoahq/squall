...
void SRgnCombineRecti(HSRGN handle, const RECTI* rect, void* param, int32_t combineMode) {
    STORM_VALIDATE_BEGIN;
    STORM_VALIDATE(rect);
    STORM_VALIDATE_END_VOID;
...
void SRgnGetBoundingRecti(HSRGN handle, RECTI* rect) {
    STORM_VALIDATE_BEGIN;
    STORM_VALIDATE(rect);
    STORM_VALIDATE_END_VOID;
...
void SRgnGetRectParamsi(HSRGN handle, const RECTI* rect, uint32_t* numParams, void** buffer) {
    STORM_VALIDATE_BEGIN;
    STORM_VALIDATE(rect);
    STORM_VALIDATE_END_VOID;
...
static_assert(sizeof(RECTI::left) == sizeof(RECTF::left), "Architecture not supported for this jank implementation");
static_assert(sizeof(RECTI) == sizeof(RECTF), "Architecture not supported for this jank implementation");

void SRgnGetRectsi(HSRGN handle, uint32_t* numRects, RECTI* buffer) {
...
int32_t SRgnIsRectInRegioni(HSRGN handle, const RECTI* rect) {
    STORM_VALIDATE_BEGIN;
    STORM_VALIDATE(rect);
    STORM_VALIDATE_END;
