...
int32_t STORMAPI STransCombineMasks(HSTRANS basemask, HSTRANS secondmask, int32_t offsetx, int32_t offsety, uint32_t flags, HSTRANS* handle) {
    ...
    bool intersect = (flags & 1) != 0;
    bool invertsecond = (flags & 2) != 0;

    bool usespan[NUM_OPS][NUM_OPS][NUM_OPS];
    for (int spantype0 = 0; spantype0 < NUM_OPS; spantype0++) {
        for (int spantypedest = 0; spantypedest < NUM_OPS; spantypedest++) {
            if (intersect) {
    ...
    for (int line = 0; line < basemask->height; line++) {
        ...
        if (line >= offsety && line <= offsety + secondmask->height - 1) {
            ...
            if (offsetx < 0) {
                ...
                while (bytesleft) {
                    ...
                    for (int adjustremaining = 0; adjustremaining < NUM_OPS; adjustremaining++) {
                        int adjustment = std::min(bytesleft, span[1][adjustremaining]);
                    ...
                    while(span[0][j]) {
                        ...
                        for (int k = 0; k < NUM_OPS; k++) {
                            ...
                            for (int m = 0; m < NUM_OPS; m++) {
                                int v11 = std::min(span[1][m], spanlength);
                                span[1][m] -= v11;
                                spanlength -= v11;
                            }
                        }
            ...
            *dest++ = 0;
            ...
            int width = basemask->width;
            while (width) {
                int skipspan = std::min(width, MAXSPANLENGTH);
                *dest++ = 0;
                *dest++ = skipspan;
                buffer.bytesused += 2;
