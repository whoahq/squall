...
int32_t SBmpAllocLoadImage(const char* filename, STORM_PALETTEENTRY* paletteentries, uint8_t** returnedbuffer, uint32_t* width, uint32_t* height, uint32_t* bitdepth, uint32_t requestedbitdepth, SBMPALLOCPROC allocproc) {
...
int32_t SBmpDecodeImage(int32_t imagetype, uint8_t* imagedata, uint32_t imagebytes, STORM_PALETTEENTRY* paletteentries, uint8_t* bitmapbits, uint32_t buffersize, uint32_t* width, uint32_t* height, uint32_t* bitdepth) {
    if (width) *width = 0;
    if (height) *height = 0;
    if (bitdepth) *bitdepth = 0;
...
int32_t SBmpLoadImage(const char* filename, STORM_PALETTEENTRY* paletteentries, uint8_t* bitmapbits, uint32_t buffersize, uint32_t* width, uint32_t* height, uint32_t* bitdepth) {
    if (width) *width = 0;
    if (height) *height = 0;
    if (bitdepth) *bitdepth = 0;
