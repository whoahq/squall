...
TEST_CASE("StormSetOption", "[option]") {
    SECTION("sets options") {
        ...
        if (optname == STORM_OPTION_SMEM_COUNTS) {
    ...
    SECTION("fails if wave chunk size is not a power of 2") {
        ...
        INFO(optval);
        CHECK_FALSE(StormSetOption(STORM_OPTION_WAV_CHUNK_SIZE, &optval, 4));
    }

    SECTION("succeeds if wave chunk size is a power of 2") {
        uint32_t optval = GENERATE(1, 2, 4, 8, 16, 32, UINT_MAX / 2 + 1);
        INFO(optval);
        CHECK(StormSetOption(STORM_OPTION_WAV_CHUNK_SIZE, &optval, 4) == 1);
    }
    ...
    SECTION("fails for any len not matching") {
        ...
        int32_t optval = 999;
        CHECK_FALSE(StormSetOption(STORM_OPTION_SMEM_COUNTS, &optval, optlen));
        CHECK(SErrGetLastError() == ERROR_INVALID_PARAMETER);
    }

// The DLL versions always succeeds invalid opcodes, different from WoW
#if defined(WHOA_BAD_STORM_OPTION_ALWAYS_SUCCEEDS)
    SECTION("succeeds if optname is invalid or unsupported") {
        int32_t optname = GENERATE(0, -1, STORM_OPTION_CD_THREAD, LAST_OPTION + 1);
...
#else
    SECTION("fails if optname is invalid or unsupported") {
        int32_t optname = GENERATE(0, -1, STORM_OPTION_CD_THREAD, LAST_OPTION + 1);
...
TEST_CASE("StormGetOption", "[option]") {
    ...
    SECTION("gets alloc and free count") {
        ...
        StormGetOption(STORM_OPTION_SMEM_COUNTS, &optval, &optlen);
    ...
    SECTION("fails for any len too small") {
        ...
        int32_t optval = 999;
        CHECK_FALSE(StormGetOption(STORM_OPTION_SMEM_COUNTS, &optval, &optlen));
        CHECK(optval == 999);
        CHECK(SErrGetLastError() == ERROR_INVALID_PARAMETER);
    }
